 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "The payoff of a European call option $ C_T $ at expiration $ T $ is given by:\n",
    "\n",
    "$$\n",
    "C_T = \\max(I_T - K, 0),\n",
    "$$\n",
    ...
    "\n",
    "#### European Put Option\n",
    "The payoff of a European put option $ P_T $ at expiration $ T $ is given by:\n",
    "\n",
    "$$\n",
    "P_T = \\max(K - I_T, 0).\n",
    "$$\n",
    ...
    "#### Call Option\n",
    "$$\n",
    "C_0 = I_0 N(d_1) - K e^{-rT} N(d_2).\n",
    "$$\n",
    "\n",
    "#### Put Option\n",
    "$$\n",
    "P_0 = K e^{-rT} N(-d_2) - I_0 N(-d_1),\n",
    "$$\n",
    "\n",
    "where:\n",
    ...
    "\n",
    "$$\n",
    "d_1 = \\frac{\\log \\left(\\frac{I_0}{K}\\right) + \\left(r + \\frac{\\sigma^2}{2} \\right)T}{\\sigma\\sqrt{T}},\n",
    "$$\n",
    "\n",
    "$$\n",
    ...
    "- Terminal condition: $ V_{i,N} = \\max(K - i \\Delta I, 0). $\n",
    "- Boundary conditions: $ V[0,j] = K e^{-r(T-j \\Delta t)} $ and $ V[M,j] = 0 .$\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        \"\"\"\n",
    "        Runs the explicit finite difference scheme for the European put option under the Black-Scholes model.\n",
    "        \n",
    "        Parameters:\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    fd_price = round(option_pricing.fd_european_put(), 2)\n",
    "    analytical_price = round(option_pricing.analytical_european_put_price(), 2)\n",
    "    mc_price = round(option_pricing.monte_carlo_european_put_price(), 2)\n",
    "    \n",
    "    # Calculate relative errors\n",
    "    fd_error = round(abs((fd_price - analytical_price) / analytical_price) * 100, 2)\n",
    "    mc_error = round(abs((mc_price - analytical_price) / analytical_price) * 100, 2)\n",
    "    \n",
   ...
   "outputs": [
    {
     ...
     "output_type": "stream",
     "text": [
      "    I_0     K  Analytical Price  FD Price  MC Price  FD Error (%)  \\\n",
      "0  1000  1000             85.21     85.13     83.73          0.09   \n",
      "1  1100  1200            131.91    131.83    132.49          0.06   \n",
      "2  1200  1100             70.75     70.70     69.53          0.07   \n",
      "3  2000  2000            170.43    170.25    174.36          0.11   \n",
      "4  2200  2300            223.95    223.94    221.52          0.00   \n",
      "5  2500  2400            179.84    179.80    181.66          0.02   \n",
      "\n",
      "   MC Error (%)  \n",
      "0          1.74  \n",
      "1          0.44  \n",
   ...
   "outputs": [
    {
     ...
     "output_type": "stream",
     "text": [
      "Time taken for the calculations: 4.02 minutes\n"
     ]
    }
   ],
   "source": [
    "print(f\"Time taken for the calculations: {elapsed_time:.2f} minutes\")"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    "## 5. Conclusion\n",
    "\n",
    "In this notebook, we have explored three different methods for valuing European put options: the explicit finite difference method, the BS analytical formula, and Monte Carlo simulation. \n",
    "\n",
    "For a range of initial index prices $ I_0 $ and strike prices $ K $, we calculated the option prices using all three methods as presented in the Numerical Example section. The FD scheme is more closer to the BS formula than the MC method as shown by the relative errors. On the MC method, we can increase the number of simulations to make the values more accurate, however, that will require more computational time.\n",
    "\n",
    "### Summary and Stuff:\n",
    "1. **Finite Difference Method**: This method provides a numerical solution to the BS PDE. It is flexible in a way that it can handle various boundary conditions, but it requires careful consideration of stability and convergence. Hence, we use the stability condition to compute the time steps.\n",
